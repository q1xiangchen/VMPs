...
def temporal_sampling(frames, start_idx, end_idx, num_samples):
    ...
    """
    index = torch.linspace(start_idx, end_idx, num_samples).long()
    # index = torch.clamp(index, 0, frames.shape[0] - 1).long()
    frames = torch.index_select(frames, 0, index)

    # frame_diff_check = [(frames[i] == frames[i+1]).all().item() for i in range(frames.shape[0]-1)]
    # tmp = sum(frame_diff_check)

    # if tmp != 0:
    #     print("*"*70)
    #     print(f"DataLoader: frame diff check: {tmp + 1 if tmp != 0 else 0} same frames\n {frame_diff_check}\nindex: {index}\n")
    #     # print the same frames
    #     for i, check in enumerate(frame_diff_check):
    #         if check:
    #             print(frames[i, 0, :5, 0])
    #             print(frames[i+1, 0, :5, 0])
    #     print("*"*70)
    return frames
...
def decode(
    ...
    if frames.size(0) < num_frames:
        repeat_time = math.ceil(num_frames / frames.size(0))
        frames = frames.repeat(repeat_time, 1, 1, 1)
        start_idx = 0
        end_idx = frames.size(0) - 1
    elif frames.size(0) - 1 < clip_sz:
        start_idx = 0
        end_idx = frames.size(0) - 1
