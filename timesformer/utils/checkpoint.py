...
def get_last_checkpoint(path_to_job, model_index):
    """
    ...
    model_indices = [int(sorted(names)[i].split("_")[2].split(".")[0]) for i in range(len(names))]
    if model_index not in model_indices:
        print(f"model_index: {model_index} not found in {model_indices}")
        return None
    idx = model_indices.index(model_index)
    name = sorted(names)[idx]
    print(f"model_index: {model_index}, file_name: {name}")
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR, cfg.MODEL_INDEX)
